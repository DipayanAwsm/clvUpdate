...
from training.pnc_clv_engine import add_pnc_clv_columns
...
def _derive_insurance_clv_target(data: pd.DataFrame, messages: List[str]) -> Dict[str, Any]:
    """Derive the P&C CLV target used by the regression model.

    Manager-visible formula implemented in `training/pnc_clv_engine.py`:
        ...
        P&C CLV = sum(
            ((Premium - Expected Claims - Expenses) * Survival)
            / ((1 + discount_rate) ** year)
            for year in 1..projection_years
        )

    `retention_probability` is generated by a renewal propensity model trained
    on `policy_renewed_flag` when that target is available.
    """
    updated_data, meta = add_pnc_clv_columns(
        data,
        model_dir=MODELS_ROOT,
        train_renewal_model=True,
    )

    for col in updated_data.columns:
        data[col] = updated_data[col]

    data["profit"] = data["annual_profit"]

    messages.append(
        "Derived P&C CLV target using direct written premium, expected claims, "
        "tax, commission expense, service expense, renewal propensity survival, "
        "and discounting. Trained/used renewal propensity model for retention_probability."
    )
    return meta
